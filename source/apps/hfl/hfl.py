...
# Client-Edge-Cloud Hierarchical Federated Learning Architecture

class CECTaskType(TaskType):
...
class CECConfig(Config):
    def __init__(self,
        data_dir: str, task_type: CECTaskType = CECTaskType.SC,
        cloud_epochs: int=100, edge_epochs:int =10, client_epochs: int=2,
        client_num: int=350, batch_size: int=10, lr: float=0.01,
        device: str="cpu",
        ):
        super().__init__(data_dir, task_type, client_num, batch_size, lr, client_epochs, device)
...
class HFL(App):
    ...
    def __init__(self, config: CECConfig):
        ...
        if self.config.task_type == CECTaskType.SC:
...
if __name__ == "__main__":
    config = CECConfig("./dataset/raw", CECTaskType.SC, 
